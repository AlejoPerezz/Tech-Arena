...
const generateScenarioFromOpenRouter = async (language) => {
  const prompt =
    language === "es"
      ? "Genera un escenario único y variado para un juego de decisiones técnicas, de nivel básico pero interesante, que cubra distintas áreas de programación (algoritmos, estructuras de datos, back-end, front-end, bases de datos, infraestructura, seguridad, rendimiento, debugging, QA/testing, DevOps, arquitectura). Evita preguntas consecutivas sobre el mismo tema y no hagas continuidad de una ronda a otra. Devuelve SOLO JSON con las claves: id (slug corto), title, prompt, hint (una pista breve), options (3 elementos). Cada opción debe tener id, label, points (entero entre -5 y 10), outcome (frase corta), explanation (una oración) y topics (2-3 temas). Responde en español."
      : "Generate one unique, varied scenario for a tech decision game, beginner-friendly but interesting, spanning different programming areas (algorithms, data structures, back-end, front-end, databases, infrastructure, security, performance, debugging, QA/testing, DevOps, architecture). Avoid consecutive questions on the same topic and do not build continuity across rounds. Return ONLY JSON with keys: id (short slug), title, prompt, hint (short hint), options (3 items). Each option must include id, label, points (integer -5 to 10), outcome (short phrase), explanation (one sentence), and topics (2-3 topics). Respond in English.";
